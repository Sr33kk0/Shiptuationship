...
| Path | Purpose |
|------|---------|
| `emails/{id}/activity/{eventId}` | Immutable audit record per moderator action: `moderator_id`, `action` (`review_saved` / `cleared` / `marked_read`), `edited_side`, `changes` (per document, per field before and after: `{ si: { shipper: { before, after } }, bl: {…} }`; records from before SI and BL were saved together hold one flat side), `before`, `after`, `occurred_at` (server time). Feeds the **User Log** |
...
- **Passwords:** hashed, not encrypted, so not even the app can recover one. `npm run hash-password` runs **scrypt** (Node's built-in `node:crypto`, default cost N=16384, r=8, p=1) with a random 16-byte salt per password and a 64-byte key, and prints `salt:hash` in hex for the `password_hash` field. At log in, `checkPassword` (`lib/session.ts`) re-hashes the typed password with the stored salt and compares with `timingSafeEqual`, so the response time does not leak how close a guess was. The password itself travels as JSON in the log in request, so production must be served over HTTPS.
